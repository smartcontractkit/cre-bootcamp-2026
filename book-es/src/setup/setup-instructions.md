# Instrucciones de Configuración

Completa estos pasos **antes** de que comience el bootcamp para asegurar una experiencia fluida.

## Este Libro

Este libro está disponible en:

```bash
https://smartcontractkit.github.io/cre-bootcamp-2026
```

## Prerequisitos Importantes

Para aprovechar al máximo este bootcamp, te recomendamos tener lo siguiente preparado **antes del Día 1**. Parte de esto se cubrirá brevemente para poder dedicar más tiempo a construir.

### Configuración Requerida

- **Node.js v20 o superior** - [Descargar aquí](https://nodejs.org/)
- **Bun v1.3 o superior** - [Descargar aquí](https://bun.sh/docs/installation)
- **CRE CLI** - [Instrucciones de instalación](https://docs.chain.link/cre/getting-started/cli-installation)
- **Foundry** - [Instrucciones de instalación](https://book.getfoundry.sh/getting-started/installation)
- **Agregar la red Ethereum Sepolia a tu wallet** - [Agregar red aquí](https://chainlist.org/chain/11155111)
- **Obtener ETH de Ethereum Sepolia del faucet** - [Chainlink Faucet](https://faucets.chain.link/sepolia)
- **Clave API de Gemini LLM** - [Obtener desde Google AI Studio](https://aistudio.google.com/apikey)

### Opcional

- **Instalar mdBook** - Para poder compilar y leer la documentación localmente
  ```bash
  cargo install mdbook
  ```

## Repositorio de Referencia

El proyecto completo del bootcamp está disponible como referencia:

```bash
https://github.com/smartcontractkit/cre-bootcamp-2026
```

> **Nota**: No necesitas clonarlo! Durante el bootcamp, construiremos todo desde cero. El repositorio está ahí por si te trabas o quieres comparar tu código.
