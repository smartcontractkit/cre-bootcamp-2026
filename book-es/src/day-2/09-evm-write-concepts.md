...
La capacidad **EVM Write** permite que tu flujo de trabajo CRE escriba datos en smart contracts en blockchains compatibles con EVM. 

> Este es uno de los patrones más importantes en CRE.

## Familiarizate con la capacidad

La capacidad EVM Write permite que tu flujo de trabajo envie reportes firmados criptográficamente a smart contracts. 

A diferencia de las aplicaciones web3 tradicionales que envian transacciones directamente, CRE usa un proceso seguro de dos pasos:

1. **Generar un reporte firmado** - Tus datos son codificados en ABI y envueltos en un "paquete" firmado criptográficamente
2. **Enviar el reporte** - El reporte firmado se envia a tu contrato consumidor a través del `KeystoneForwarder` de Chainlink
...
Mira como hacerlo para el mercado que obtuvimos del HTTP Trigger:
...
Como enviar el reporte firmado al contrato consumidor:
...
Mira como configurar la red blockchain y crear el cliente EVM:
...
Ahora hagamoslo en el proyecto Prediction Market!
